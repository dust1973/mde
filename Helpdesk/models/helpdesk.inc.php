...
class Helpdesk
{
    ...
    {
        $parm[] = new SoapVar('<ResponseMessages>', XSD_ANYXML, null, null, null, 'https://redurunner.zentrale-woehrl.de/HelpdeskRedurunner/');
        $parm[] = new SoapVar('<Pong>1</Pong>', XSD_ANYXML, null, null, 'Pong', 'https://redurunner.zentrale-woehrl.de/HelpdeskRedurunner/');
        $parm[] = new SoapVar('</ResponseMessages>', XSD_ANYXML, null, null, null, 'https://redurunner.zentrale-woehrl.de/HelpdeskRedurunner/');
        $rueckgabewert = new SoapVar($parm, SOAP_ENC_OBJECT, 'object', 'https://redurunner.zentrale-woehrl.de/HelpdeskRedurunner/', 'Result');
    ...
    function GetSettings($parameters)
    {
        ...
        if ($rueckgabewert = $abfrage->fetchObject()) {
            ...
            $parm[] = new SoapVar('<Pin>'.$parameters->Pin.'</Pin>', XSD_ANYXML);
            $parm[] = new SoapVar('</Parameters>', XSD_ANYXML);
            $rueckgabewert = new SoapVar($parm, SOAP_ENC_OBJECT, null, null, 'Result');
...
VALUES (:pin, :debug,  :debugLevel, :url_helppage, :url_startpage, :scanner, :drucker)";
            ...
            */
            ...
            if ($abfrage->execute()) {
                ...
                $parm[] = new SoapVar('Daten wurden erfolgreich ge�ndert.</Success></Parameters>', XSD_ANYXML);
                $rueckgabewert = new SoapVar($parm, SOAP_ENC_OBJECT, null, null, 'Debug');
                return $rueckgabewert;
    ...
    function GetDebugRecord($parameters)
    {
        ...
        $sql = "SELECT log FROM fingerscanner.log_redurunner WHERE pin = :pin ORDER BY zeitstempel DESC";
       ...
       // $parameters = array('test');
            $parm[] = new SoapVar('<Parameters>', XSD_ANYXML);
            $parm[] = new SoapVar('<Logs>' . serialize($rueckgabewert) . '</Logs>', XSD_ANYXML);
            $parm[] = new SoapVar('</Parameters>', XSD_ANYXML);
            $rueckgabewert = new SoapVar($parm, SOAP_ENC_OBJECT, null, null, 'Result');
        ...
        if ($rueckgabewert[0]['ErrMsg']) {
            ...
            switch ($rueckgabewert[0]['ErrMsg']) {
                case -1:
                    ...
                    break;
                case -2:
                    $parm[] = new SoapVar('Fehler-ung�ltiger Status!', XSD_STRING, null, null, 'Fehler');
                    $rueckgabewert = new SoapVar($parm, SOAP_ENC_OBJECT);
                    return $rueckgabewert;
                    break;
                default:
            ...
            $parm[] = new SoapVar('<Filialbezeichnung>' . $rueckgabewert[0]['Filialbezeichnung'] . '</Filialbezeichnung>', XSD_ANYXML);
            ...
            $parm[] = new SoapVar('<Flaechengruppe>' . utf8_encode($rueckgabewert[0]['Flaechengruppe']) . '</Flaechengruppe>', XSD_ANYXML);

            $parm[] = new SoapVar('<Lieferant>' . utf8_encode($rueckgabewert[0]['Lieferant']) . '</Lieferant>', XSD_ANYXML);
            $parm[] = new SoapVar('<Mitarbeiter>' . utf8_encode($rueckgabewert[0]['Mitarbeiter']) . '</Mitarbeiter>', XSD_ANYXML);
            $parm[] = new SoapVar('<Abschriftenart>' . utf8_encode($rueckgabewert[0]['Abschriftenart']) . '</Abschriftenart>', XSD_ANYXML);
            $parm[] = new SoapVar('<Bemerkung>' . utf8_encode($rueckgabewert[0]['Bemerkung']) . '</Bemerkung>', XSD_ANYXML);
            $parm[] = new SoapVar('<Kategorie>' . utf8_encode($rueckgabewert[0]['Kategorie']) . '</Kategorie>', XSD_ANYXML);
            ...
            foreach ($rueckgabewert as $rueckgabewer) {
                ...
                $parm[] = new SoapVar('<Artikelbezeichnung>' . utf8_encode(htmlentities($rueckgabewer['ArtikelBezeichnung'])) . '</Artikelbezeichnung>', XSD_ANYXML);
                ...
                $parm[] = new SoapVar('<KredFarbe>' . utf8_encode($rueckgabewer['KredFarbe']) . '</KredFarbe>', XSD_ANYXML);
                $parm[] = new SoapVar('<Strichcode>' . $rueckgabewer['Strichcode'] . '</Strichcode>', XSD_ANYXML);
                $parm[] = new SoapVar('<Farbe>' . utf8_encode($rueckgabewer['Farbe']) . '</Farbe>', XSD_ANYXML);
                ...
                $parm[] = new SoapVar('<ModellNr>' . $rueckgabewer['ModellNr'] . '</ModellNr>', XSD_ANYXML);
